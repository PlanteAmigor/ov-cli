...
/**
 * @brief Forces </think> when reasoning budget is exhausted.
 *
 * State machine:
 *   IDLE        → waiting for thinking_start_token_id
 *   COUNTING    → counting tokens inside thinking block
 *   FORCING     → all logits = -inf except thinking_end_token_id
 *   DONE        → passthrough forever (after end token emitted)
 */
...
public:
    ...
    ThinkingBudgetTransform(int64_t budget, int64_t start_id, int64_t end_id)
        : m_budget(budget), m_start_id(start_id), m_end_id(end_id), m_state(IDLE), m_count(0) {
        // When prompt already contains <think> (as most Qwen chat templates do),
        // the start token won't be generated again, so we start in COUNTING state.
        // Budget=0 means force </think> immediately on first apply().
        m_state = COUNTING;
        m_count = 0;
    }
    ...
    void accept_token(int64_t token_id) {
        ...
        if (m_state == COUNTING) {
            ...
            ++m_count;
            if (m_count >= static_cast<size_t>(m_budget)) {
                m_state = FORCING;
            }
            return;
        }

        // FORCING: once we've forced the end token, transition to DONE
        if (m_state == FORCING) {
            if (token_id == m_end_id) {
                m_state = DONE;
            }
    ...
    bool is_applicable(size_t /*generated_tokens_cnt*/) override {
        return true;
    }
